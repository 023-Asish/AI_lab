...
def checkCurrentPosition(world,i,j):
...
def checkAdjacentPosition(agi,agj):
...
if(agvi==1 and agvj==0 or agvi==0 and agvj==1):
	agi,agj=checkCurrentPosition(world,agvi,agvj)				#if input valid calling learn agent function
else:
	print("Not valid")								

while(agi>=0):
	agvi,agvj=checkAdjacentPosition(agi,agj)
	if(agvi!=-5 and agvj!=-5):
		agi,agj=checkCurrentPosition(world,agvi,agvj)			
	else:
		print("\nNot valid")

if(agi==-5):
	print("\nYou are Eatten by wumpus!!!")
else:
